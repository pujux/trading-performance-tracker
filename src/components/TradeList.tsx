      ...
      <table className="min-w-full overflow-hidden bg-white rounded-lg">
        <thead className="bg-gray-50">
          <tr>
            ...
            <th className="px-6 py-3 text-xs font-medium tracking-wider text-center text-gray-500 uppercase">Symbol</th>
            <th className="px-6 py-3 text-xs font-medium tracking-wider text-center text-gray-500 uppercase">Type</th>
            <th className="px-6 py-3 text-xs font-medium tracking-wider text-center text-gray-500 uppercase">Category</th>
            <th className="px-6 py-3 text-xs font-medium tracking-wider text-center text-gray-500 uppercase">Avg Entry</th>
            <th className="px-6 py-3 text-xs font-medium tracking-wider text-center text-gray-500 uppercase">Avg Exit</th>
            <th className="px-6 py-3 text-xs font-medium tracking-wider text-center text-gray-500 uppercase">Total Qty</th>
            <th className="px-6 py-3 text-xs font-medium tracking-wider text-center text-gray-500 uppercase">Open Qty</th>
            <th className="px-6 py-3 text-xs font-medium tracking-wider text-center text-gray-500 uppercase">Order Costs</th>
            <th className="px-6 py-3 text-xs font-medium tracking-wider text-center text-gray-500 uppercase">P&L</th>
            <th className="px-6 py-3 text-xs font-medium tracking-wider text-center text-gray-500 uppercase">Holding Period</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-gray-200">
          {sortedTrades.length === 0 && (
            <tr>
              <td colSpan={11} className="p-6 text-center text-gray-900">
            ...
            return (
              <Fragment key={trade.id}>
                <tr className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-sm text-right text-gray-500 whitespace-nowrap">{avgEntryPrice.toFixed(2)}</td>
                  <td className="px-6 py-4 text-sm text-right text-gray-500 whitespace-nowrap">{avgExitPrice.toFixed(2)}</td>
                  <td className="px-6 py-4 text-sm text-right text-gray-500 whitespace-nowrap">{totalEntryQty}</td>
                  <td className="px-6 py-4 text-sm text-right text-gray-500 whitespace-nowrap">{openQty}</td>
                  <td className="px-6 py-4 text-sm text-right text-gray-500 whitespace-nowrap">{totalOrderCosts.toFixed(2)}</td>
                  <td className="px-6 py-4 text-sm text-right text-gray-500 whitespace-nowrap">
                    <span className={openQty === 0 ? (pnl >= 0 ? "text-green-600" : "text-red-600") : ""}>
                      {openQty === 0 ? pnl.toFixed(2) : "..."}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-sm text-right text-gray-500 whitespace-nowrap" title={holdingPeriod.fullPeriod}>
                    ...
                    <td colSpan={11} className="p-8 bg-gray-50">
                      <div className="flex flex-col gap-4">
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                          <thead>
                            <tr>
                              <th className="px-4 py-2 text-xs font-medium text-center text-gray-500">Type</th>
                              <th className="px-4 py-2 text-xs font-medium text-right text-gray-500">Price</th>
                              <th className="px-4 py-2 text-xs font-medium text-right text-gray-500">Quantity</th>
                              <th className="px-4 py-2 text-xs font-medium text-right text-gray-500">Order Cost</th>
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-gray-200">
